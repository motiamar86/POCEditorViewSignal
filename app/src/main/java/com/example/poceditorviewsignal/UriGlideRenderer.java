...
import com.example.poceditorviewsignal.util.BitmapUtil;
...
public final class UriGlideRenderer implements Renderer {
      ...
      if (rendererContext.isBlockingLoad()) {
        ...
        getBitmapGlideRequest(rendererContext.context, true).into(new CustomTarget<Bitmap>() {
          ...
          public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
            ...
            rendererContext.invalidate.onInvalidate(
  ...
  private void setBitmap(@NonNull RendererContext rendererContext, @Nullable Bitmap bitmap) {
    ...
    if (bitmap != null) {
      ...
      rendererContext.rendererReady.onReady(
              UriGlideRenderer.this, cropMatrix(bitmap), new Point(bitmap.getWidth(), bitmap.getHeight()));
    }
  }
  ...
  public static final Parcelable.Creator<UriGlideRenderer> CREATOR = new Parcelable.Creator<UriGlideRenderer>() {
    @Override
    public
    UriGlideRenderer createFromParcel(Parcel in) {
      return new
              UriGlideRenderer(Uri.parse(in.readString()),
                                  in.readInt() == 1,
                                  in.readInt(),
                                  in.readInt()
                                 );
    ...
    @Override
    public
    UriGlideRenderer[] newArray(int size) {
      return new
              UriGlideRenderer[size];
    }
  };
